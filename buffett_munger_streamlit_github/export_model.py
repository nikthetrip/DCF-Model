...
def build_excel_export(
    ...
    normalization_snapshot: dict[str, Any] | None = None,
) -> bytes:
    output = BytesIO()
    with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
        ...
        summary = pd.DataFrame(
            {
                "Metric": [
                    "Company", "Ticker", "Source", "Current price", "Owner earnings ($mm)",
                    "Owner earnings per share", "Owner earnings yield", "No-growth value per share",
                    "DCF intrinsic value per share", "Target buy price", "Margin of safety",
                    "Terminal value share",
                ],
                "Value": [
                    company.get("name"), company.get("symbol"), company.get("source"), company.get("price"),
                    ...
                    valuation_result["margin_of_safety"], valuation_result.get("terminal_value_share"),
                ],
            }
        )
        ...
        if financial_snapshot:
            pd.DataFrame([financial_snapshot]).to_excel(writer, sheet_name="Financial Data", index=False)
        if normalization_snapshot:
            pd.DataFrame([normalization_snapshot]).to_excel(writer, sheet_name="Normalization", index=False)
        ...
        for sheet_name in writer.sheets:
            ...
            ws.set_column(1, 40, 19)
        writer.sheets["Dashboard"].set_column("B:B", 22)
